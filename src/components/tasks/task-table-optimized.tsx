// src/components/tasks/task-table-optimized.tsx - FIXED ACTION HANDLING
'use client'
...
interface TaskTableOptimizedProps {
  tasks: Task[]
  isLoading: boolean
...
const statuses = ['TODO', 'IN_PROGRESS', 'IN_REVIEW', 'COMPLETED', 'CANCELLED'] as const

export function TaskTableOptimized({ tasks, isLoading, onEditTask }: TaskTableOptimizedProps) {
  ...
  // 🚀 PERFORMANCE: Memoize filtered tasks
  ...
  // 🚀 PERFORMANCE: Memoize pagination
  const paginationData = useMemo(() => {
    ...
    return { totalPages, startIndex, endIndex, currentTasks }
  }, [filteredTasks, currentPage, itemsPerPage])

  // 🔧 FIXED: Prevent event bubbling for actions
  const handleStatusChange = useCallback(async (e: React.MouseEvent, taskId: string, newStatus: string) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>
          Showing {paginationData.currentTasks.length} of {filteredTasks.length} tasks
                    ...
                    <TableRow key={task.id} className="hover:bg-muted/50">
                      ...
                      <TableCell>
                        ...
                        <div onClick={(e) => e.stopPropagation()}>
                          <Select 
                            value={task.status} 
                            onValueChange={(value) => handleStatusChange(new MouseEvent('click') as any, task.id, value)}
      ...
      {paginationData.totalPages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            Page {currentPage} of {paginationData.totalPages} • {filteredTasks.length} total tasks
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
